...
from ..observability.metrics import record_pass_outcome, stage_timer
...
async def run_pipeline_pass(
    *,
    ...
    try:
        async with stage_timer("load_context"):
            manifest = await load_context(xnch, session, intent)
    except Exception as exc:
        record_pass_outcome("ERROR")
    ...
    async with stage_timer("weight_config"):
        try:
    ...
    try:
        async with stage_timer("policy_filter"):
    ...
    # Step 7 — Scoring
    async with stage_timer("evaluate"):
    ...
    try:
        async with stage_timer("compile_plan"):
    ...
    try:
        async with stage_timer("submit_verdict"):
    ...
    try:
        async with stage_timer("dispatch"):
