...
# ---------------------------------------------------------------------------
# Dynamic persona overlay (live self-model) tests
# ---------------------------------------------------------------------------

_PERSONA_OVERLAY = """# AUTO-GENERATED by nexi/character/persona_builder.py — do not edit.
...
self_model:
  ...
  services: []
  mcp_servers: [code-review-graph, xnch, superpowers]
  skills: [{name: debug-issue}, {name: brainstorming}]
  tool_count: 37
  repo_packages: [xnch, nexi, infra]
  backend_health: {opencode-go: true}
"""


def test_load_persona_merges_persona_overlay(monkeypatch, tmp_path):
    overlay = tmp_path / "persona.generated.yaml"
    overlay.write_text(_PERSONA_OVERLAY)
    monkeypatch.setattr(
        "nexi.character.prompt_loader.settings.persona_generated_path", str(overlay)
    )
    persona = load_persona()
    assert "self_model" in persona
    assert persona["self_model"]["active_model"] == "deepseek-v4-pro"


def test_build_system_prompt_renders_live_persona(monkeypatch, tmp_path):
    overlay = tmp_path / "persona.generated.yaml"
    overlay.write_text(_PERSONA_OVERLAY)
    monkeypatch.setattr(
        "nexi.character.prompt_loader.settings.persona_generated_path", str(overlay)
    ...
    assert "37 live tools" in prompt
    assert "code-review-graph, xnch, superpowers" in prompt
    # no unresolved placeholders leaked into the live prompt
    assert "{" not in prompt


def test_load_persona_missing_overlay_no_self_model(monkeypatch, tmp_path):
    monkeypatch.setattr(
        "nexi.character.prompt_loader.settings.persona_generated_path",
        str(tmp_path / "does-not-exist.yaml"),
    )
    persona = load_persona()
    assert "self_model" not in persona
    # Degraded mode still builds a prompt (raw template, not crash)
    prompt = build_system_prompt()
    assert "You are Nexi" in prompt


def test_load_persona_bad_overlay_falls_back(monkeypatch, tmp_path):
    overlay = tmp_path / "persona.generated.yaml"
    overlay.write_text("::: not valid yaml")
    monkeypatch.setattr(
        "nexi.character.prompt_loader.settings.persona_generated_path", str(overlay)
